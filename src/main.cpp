...
#define sleepPinTuoi 4
#define resetPinTuoi 16
#define enPinTuoi 17
#define maiche 15
#define led 33
#define stepPinHo 27
...
void setup() {
  ...
  pinMode(mua, INPUT);
  pinMode(dirPinTuoi, OUTPUT);
  pinMode(stepPinTuoi, OUTPUT);
  pinMode(sleepPinTuoi, OUTPUT);
  pinMode(resetPinTuoi, OUTPUT);
  pinMode(maiche, OUTPUT);
...
void loop() {
  ...
  if (millis() - lastSend > 2000) {
        ...
        !isnan(khoangcach) && !isnan(trangthai_mua)) 
    {
      ...
      Serial.print("Chuyen dong: "); Serial.print(chuyendong_value); Serial.print(", ");
